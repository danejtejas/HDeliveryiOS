...
protocol ItemRepositoryProtocol {
    func getItems(itemRequest: GetItemsRequest) -> AnyPublisher<[DeliveryItem], Error>
        ...
        return networkClient.executePublisher(itemRequest)
            ...
            .map { response in
                if response.isSuccess {
                    ...
                    print("Items API Error: \(String(describing: response.message))")
                    return []
                }
            }
            .catch { error -> AnyPublisher<[DeliveryItem], Error> in
